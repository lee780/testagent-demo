...
> 归档用途：描述 TestPilot 测试平台、授信 C 系统、挡板服务、各自数据库、外部真实三方渠道的部署位置、网络关系与调用流向。
...
> 切换点：C 系统配置文件中 `STUB_SERVER_URL` 指向挡板地址，**C 系统代码零修改**。
...
| 组件 | 所属系统 | 网络位置 | 端口 |
|------|----------|----------|------|
| TestPilot Agent | TestPilot 测试平台 | 内网 | — |
| TestPilot DB | TestPilot 测试平台 | 内网 | 5432 |
| 授信 C 系统 | 被测系统 | 内网 | 8000 |
| C 系统 DB | 被测系统 | 内网 | 5432（独立实例） |
| 挡板系统 | 测试基础设施 | 内网 | 8002 |
| 真实外部三方接口 | 外网 | 外网 | — （测试不可达） |

> TestPilot DB 与 C 系统 DB 是**两个完全独立的数据库实例**，职责不同：
> - **TestPilot DB**：存储平台自身数据（对话、报告、用例、缺陷、知识库）
> - **C 系统 DB**：存储被测系统的客户存量数据（历史流水、已有指标），Agent 在测试前往这里写入前置数据来控制分支
