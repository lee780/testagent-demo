...
<RESPONSE>
  ...
  <credit_limit>103500.00</credit_limit>
</RESPONSE>
```
...
> `avg_3m_balance` 以**千元**为单位参与计算（原值÷1000），计算结果为负数时返回 `0.00`（归零处理）。

### 4.4 计算示例

| user_level | avg_3m_balance | monthly_salary | 外部指标 | 系数 | credit_limit |
|---|---|---|---|---|---|
| 2 | 1500.00 | 15000 | 全部正常 | 2.3（>1000） | `2×(1500/1000)×15000×2.3=103500.00` |
| 2 | 500.00 | 15000 | 全部正常 | 0.5（0<≤1000） | `2×(500/1000)×15000×0.5=7500.00` |
| 1 | -5000.00 | 15000 | 全部正常 | 0.2（≤0） | `1×(-5000/1000)×15000×0.2=-15000→0.00`（归零） |
